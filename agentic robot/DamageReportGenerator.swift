    ...
    static func generatePDF(
        ...
        do {
            try renderer.writePDF(to: url) { context in
                ...
                func numericBarcodeID() -> String {
                    // Prefer the ID generated by ReportStore so the barcode value
                    // exactly matches the Firestore document ID used for lookup.
                    // Keep only digits so Code 128 and manual lookup stay clean.
                    if let numericBarcodeId {
                        let digitsOnly = numericBarcodeId.filter { $0.isNumber }
                        if !digitsOnly.isEmpty { return digitsOnly }
                    }

                    // Fallback for older call sites. Use UInt64-style accumulation
                    // so signed integer overflow can never create a negative ID.
                    let hash = cleanedPlate.unicodeScalars.reduce(UInt64(0)) { acc, scalar in
                        (acc &* 31 &+ UInt64(scalar.value)) % 10_000
                    }
                    let plateStr = String(format: "%04llu", hash)
                    return "\(compactDate)\(plateStr)"
                }

                func drawBarcode(in rect: CGRect) {
                    let code = numericBarcodeID()   // always 12 pure digits, never negative
                    ...
                    filter.setValue(8.0, forKey: "inputQuietSpace")

                    guard let ciImage = filter.outputImage else { return }

                    // Scale to fill the barcode area (top portion — leave room for digits)
                    let digitAreaH: CGFloat = 14
                    let barRect = CGRect(x: rect.minX, y: rect.minY,
                                        width: rect.width, height: rect.height - digitAreaH)

                    let scaleX = barRect.width  / ciImage.extent.width
                    let scaleY = barRect.height / ciImage.extent.height
                    ...
                    // Draw bars in the top portion
                    UIImage(cgImage: cgImage).draw(in: barRect)

                    // Draw the 12-digit number centred below the bars — standard barcode style
                    let paragraphStyle = NSMutableParagraphStyle()
                    paragraphStyle.alignment = .center
                    let attrs: [NSAttributedString.Key: Any] = [
                        .font: UIFont(name: "Courier", size: 8) ?? UIFont.monospacedDigitSystemFont(ofSize: 8, weight: .regular),
                        .foregroundColor: UIColor.black,
                        .paragraphStyle: paragraphStyle
                    ]
                    let digitRect = CGRect(x: rect.minX, y: rect.maxY - digitAreaH,
                                          width: rect.width, height: digitAreaH)
                    (code as NSString).draw(in: digitRect, withAttributes: attrs)
                }
                ...
                func drawHeader(pageText: String) {
                    ...
                    // Barcode: wide and short, digits printed inside the rect by drawBarcode()
                    let barcodeW: CGFloat = 175
                    let barcodeH: CGFloat = 55   // bars (41pt) + digit row (14pt)
                    drawBarcode(in: CGRect(x: right - barcodeW, y: top + 2, width: barcodeW, height: barcodeH))
                ...
                func drawBottomSignatureBlock(y: CGFloat) {
                    ...
                    if let signature = stageTwo.officerSignature {
                        // Keep the signature safely inside the first row, above the separator line.
                        signature.draw(in: CGRect(x: leftCol + colWidth - 72, y: y + 16, width: 50, height: 24))
                    }
                    ...
                    if let signature = stageTwo.informantSignature {
                        // Keep the signature safely inside the first row, above the separator line.
                        signature.draw(in: CGRect(x: rightCol + colWidth - 72, y: y + 16, width: 50, height: 24))
                    }

                    // Left row 2: interpreter signature + name, matching the officer row.
                    let interpreterY = y + row1H
                    drawString(
                        "Signature Of Interpreter:",
                        in: CGRect(x: leftCol + 5, y: interpreterY + 7, width: colWidth - 86, height: 13),
                        font: .boldSystemFont(ofSize: 8.5)
                    )
                    drawString(
                        valueOrBlank(stageTwo.interpreterAvailability),
                        in: CGRect(x: leftCol + 5, y: interpreterY + 22, width: colWidth - 100, height: 20),
                        font: .boldSystemFont(ofSize: 8.5)
                    )
                    if let interpreterSignature = stageTwo.interpreterSignature {
                        // Keep the interpreter signature narrow and safely inside the row.
                        interpreterSignature.draw(in: CGRect(x: leftCol + colWidth - 72, y: interpreterY + 16, width: 50, height: 24))
                    }
                ...
                func drawSignaturePage() {
                    ...
                    let officerSigBox = CGRect(x: leftCol + colWidth - 86, y: y + 28, width: 66, height: 48)
                    if let officerSignature = stageTwo.officerSignature {
                        officerSignature.draw(in: officerSigBox)
                    }

                    // Left column, second row: interpreter signature + name, matching officer layout.
                    let interpreterY = y + topRowH
                    drawString(
                        "Signature Of Interpreter:",
                        in: CGRect(x: leftCol + 6, y: interpreterY + 8, width: colWidth - 100, height: 14),
                        font: .boldSystemFont(ofSize: 9.5)
                    )
                    drawString(
                        valueOrBlank(stageTwo.interpreterAvailability),
                        in: CGRect(x: leftCol + 6, y: interpreterY + 26, width: colWidth - 105, height: 48),
                        font: .boldSystemFont(ofSize: 9)
                    )
                    let interpreterSigBox = CGRect(x: leftCol + colWidth - 86, y: interpreterY + 28, width: 66, height: 48)
                    if let interpreterSignature = stageTwo.interpreterSignature {
                        interpreterSignature.draw(in: interpreterSigBox)
                    }
                    ...
                    let informantSigBox = CGRect(x: rightCol + colWidth - 112, y: y + 28, width: 82, height: 48)
